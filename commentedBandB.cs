...
namespace IOpUtils
{
    public class BranchAndBoundMethod
    {
		...
		// Проверяет, все ли числа в векторе являются целыми
		...
		// Проверяет, является ли целым число с заданной точностю Eps
        private bool IsInteger(double x)
        {
            return Math.Abs(Math.Round(x, 0) - x) < Eps;
        }
		
		// Вычисляет следующую задачу с нижней границей
		...
		// Вычисляет следующую задачу с верхней границей
        ...
        {
			// Рассчитываем точность и записываем начальную задачу и начальный рекорд
            ...
            while (Tasks.Any())
            {
                ...
                {	// Если в задаче не было решения, продолжаем с другими задачами
                    continue;
                }
				
				// Если стоимость вычисленной задачи меньше рекорда, продолжаем дальше
                if (dms.ResultCost <= RecordCost)
                    continue;

				// Если вектор целый, запоминаем новый рекорд
                ...
                {	// иначе добавляем две новых задачи
                    Tasks.Push(MakeLowerTask(task, dms));
                    Tasks.Push(MakeUpperTask(task, dms));
                }
            }
			
			// Если не было ни одного целого плана, выдаем ошибку
            if (!HasResult)
